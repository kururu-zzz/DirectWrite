...
namespace fbx
{
	namespace element
	{
		namespace Load
		{
			...
			XMFLOAT3 getProperty(
				...
				if (property.IsValid() && factor.IsValid()) {
					...
					return XMFLOAT3(static_cast<float>(_[0] * f), static_cast<float>(_[1] * f), static_cast<float>(_[2] * f));
				}
				return default;
			}
		}
		...
		std::shared_ptr<Material> LoadMaterial(FbxSurfaceMaterial* material)
		{
			...
			_material.diffuse = Load::getProperty(material, FbxSurfaceMaterial::sDiffuse, FbxSurfaceMaterial::sDiffuseFactor, XMFLOAT3(1, 1, 1));
			...
			/*-----修正-----*/
			CreateTexture(_material.diffuseTextureName);
